...
int main(int argc, char *argv[])
{
    ...
    {
        //se configServer fallisce, non importa perché ritorna i valori di default (possono essere parziali o totali)
    ...
    //può sovrascrivere eventuali logFile già presenti
    ...
    while(terminaOra == 0)
    {
        ...
        for(int i = 0; i <= maxFd; i++)
        {
            ...
            if (FD_ISSET(i, &readSet))
            {
                ...
                //La select può ri-metteresi in ascolto del worker 'i' perché, quest'ultimo, ha finito di gestire la richiesta.
                if(i == pipeW2M_Read)
                {
                    int pipeFdSoccket;
...
//            stampaHash(storage);
            ...
            //notifico il manager che la connessione è terminata - server per gestire SIGHUP
            int esitoNegativo = -1;
...
void gestioneRichiesteAPI(int operazioneAPI, int fdAcceptF)
{
    switch (operazioneAPI) {
        case API_OPENFILE:
            ...
            //se esito == -2, significa che è in attesa della lock, quindi si rimane in attesa
            if(esito != -2)
            {
        ...
        case API_LOCKFILE:
            ...
            //se esito == -2, significa che è in attesa della lock
            if(esitoLock != -2)
            {
                //invia eisto della richiesta (successo o insuccesso)
                WRITEN_C(fdAcceptF, &errno, sizeof(int), "API_OPENFILE: WRITEN_C")
